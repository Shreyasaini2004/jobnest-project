...
import UserInterestsOnboarding, { UserInterestsData } from './UserInterestsOnboarding';
...
export function JobSeekerSignup({ userType }: JobSeekerSignupProps) {
  ...
  const onSubmit = async (data: SignupFormValues) => {
    try {
      ...
      if (response.ok) {
        ...
        // Do not navigate yet; wait for onboarding
      } else {
        toast({
  ...
  const handleOnboardingComplete = async (onboardingData: UserInterestsData) => {
    setShowOnboarding(false);
    // Optionally send onboardingData to backend here
    try {
      const apiUrl = import.meta.env.VITE_API_URL;
      await fetch(`${apiUrl}/api/users/onboarding`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ userId: pendingUser?._id, ...onboardingData }),
      });
    } catch (e) {
      // Ignore errors for now
    }
    // Navigate to dashboard
    if (pendingUserType === 'employer') {
  ...
  return (
    <>
      ...
      {showOnboarding && (
        <UserInterestsOnboarding onComplete={handleOnboardingComplete} onSkip={handleOnboardingSkip} />
      )}
    </>
  );
}
